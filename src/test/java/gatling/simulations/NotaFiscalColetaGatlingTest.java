...
/**
 * Performance test for the NotaFiscalColeta entity.
 */
public class NotaFiscalColetaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all notaFiscalColetas")
                    .get("/api/nota-fiscal-coletas")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new notaFiscalColeta")
                        .post("/api/nota-fiscal-coletas")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"numero\": \"SAMPLE_TEXT\"" +
                                ", \"serie\": \"SAMPLE_TEXT\"" +
                                ", \"remetente\": \"SAMPLE_TEXT\"" +
                                ", \"destinatario\": \"SAMPLE_TEXT\"" +
                                ", \"metroCubico\": 0" +
                                ", \"quantidade\": 0" +
                                ", \"peso\": 0" +
                                ", \"dataEmissao\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dataSaida\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"valorTotal\": 0" +
                                ", \"pesoTotal\": 0" +
                                ", \"quantidadeTotal\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_notaFiscalColeta_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created notaFiscalColeta").get("${new_notaFiscalColeta_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created notaFiscalColeta").delete("${new_notaFiscalColeta_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the NotaFiscalColeta entity").exec(scn);
