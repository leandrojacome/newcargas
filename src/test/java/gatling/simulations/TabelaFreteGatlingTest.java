...
/**
 * Performance test for the TabelaFrete entity.
 */
public class TabelaFreteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tabelaFretes").get("/api/tabela-fretes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tabelaFrete")
                        .post("/api/tabela-fretes")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tabelaFrete_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tabelaFrete").get("${new_tabelaFrete_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created tabelaFrete").delete("${new_tabelaFrete_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TabelaFrete entity").exec(scn);
