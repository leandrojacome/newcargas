...
/**
 * Performance test for the Regiao entity.
 */
public class RegiaoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all regiaos").get("/api/regiaos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new regiao")
                        .post("/api/regiaos")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_regiao_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created regiao").get("${new_regiao_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created regiao").delete("${new_regiao_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Regiao entity").exec(scn);
