...
/**
 * Performance test for the SolicitacaoColeta entity.
 */
public class SolicitacaoColetaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all solicitacaoColetas")
                    .get("/api/solicitacao-coletas")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new solicitacaoColeta")
                        .post("/api/solicitacao-coletas")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_solicitacaoColeta_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created solicitacaoColeta").get("${new_solicitacaoColeta_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created solicitacaoColeta").delete("${new_solicitacaoColeta_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SolicitacaoColeta entity").exec(scn);
