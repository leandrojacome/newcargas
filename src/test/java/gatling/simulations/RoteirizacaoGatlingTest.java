...
/**
 * Performance test for the Roteirizacao entity.
 */
public class RoteirizacaoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all roteirizacaos").get("/api/roteirizacaos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new roteirizacao")
                        .post("/api/roteirizacaos")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"dataHoraPrimeiraColeta\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dataHoraUltimaColeta\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dataHoraPrimeiraEntrega\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dataHoraUltimaEntrega\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"valorTotal\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_roteirizacao_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created roteirizacao").get("${new_roteirizacao_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created roteirizacao").delete("${new_roteirizacao_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Roteirizacao entity").exec(scn);
