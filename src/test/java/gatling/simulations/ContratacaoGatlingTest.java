...
/**
 * Performance test for the Contratacao entity.
 */
public class ContratacaoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all contratacaos").get("/api/contratacaos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contratacao")
                        .post("/api/contratacaos")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contratacao_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contratacao").get("${new_contratacao_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created contratacao").delete("${new_contratacao_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Contratacao entity").exec(scn);
