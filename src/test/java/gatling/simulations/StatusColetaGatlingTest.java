...
/**
 * Performance test for the StatusColeta entity.
 */
public class StatusColetaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all statusColetas").get("/api/status-coletas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new statusColeta")
                        .post("/api/status-coletas")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_statusColeta_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created statusColeta").get("${new_statusColeta_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created statusColeta").delete("${new_statusColeta_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StatusColeta entity").exec(scn);
