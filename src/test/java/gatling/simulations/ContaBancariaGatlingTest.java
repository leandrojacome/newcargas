...
/**
 * Performance test for the ContaBancaria entity.
 */
public class ContaBancariaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all contaBancarias").get("/api/conta-bancarias").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contaBancaria")
                        .post("/api/conta-bancarias")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contaBancaria_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contaBancaria").get("${new_contaBancaria_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created contaBancaria").delete("${new_contaBancaria_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ContaBancaria entity").exec(scn);
