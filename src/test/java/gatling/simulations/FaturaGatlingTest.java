...
/**
 * Performance test for the Fatura entity.
 */
public class FaturaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all faturas").get("/api/faturas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new fatura")
                        .post("/api/faturas")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"tipo\": \"EMBARCADOR\"" +
                                ", \"dataFatura\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_fatura_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created fatura").get("${new_fatura_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created fatura").delete("${new_fatura_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Fatura entity").exec(scn);
