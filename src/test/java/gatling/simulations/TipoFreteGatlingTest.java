...
/**
 * Performance test for the TipoFrete entity.
 */
public class TipoFreteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tipoFretes").get("/api/tipo-fretes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tipoFrete")
                        .post("/api/tipo-fretes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nome\": \"SAMPLE_TEXT\"" + ", \"descricao\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tipoFrete_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tipoFrete").get("${new_tipoFrete_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created tipoFrete").delete("${new_tipoFrete_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TipoFrete entity").exec(scn);
