...
/**
 * Performance test for the Embarcador entity.
 */
public class EmbarcadorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all embarcadors").get("/api/embarcadors").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new embarcador")
                        .post("/api/embarcadors")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_embarcador_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created embarcador").get("${new_embarcador_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created embarcador").delete("${new_embarcador_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Embarcador entity").exec(scn);
