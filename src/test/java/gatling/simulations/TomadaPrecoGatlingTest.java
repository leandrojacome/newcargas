...
/**
 * Performance test for the TomadaPreco entity.
 */
public class TomadaPrecoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tomadaPrecos").get("/api/tomada-precos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tomadaPreco")
                        .post("/api/tomada-precos")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tomadaPreco_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tomadaPreco").get("${new_tomadaPreco_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created tomadaPreco").delete("${new_tomadaPreco_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TomadaPreco entity").exec(scn);
