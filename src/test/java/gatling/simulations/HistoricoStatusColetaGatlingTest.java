...
/**
 * Performance test for the HistoricoStatusColeta entity.
 */
public class HistoricoStatusColetaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all historicoStatusColetas")
                    .get("/api/historico-status-coletas")
                ...
                .on(
                    exec(
                        http("Get created historicoStatusColeta")
                            .get("${new_historicoStatusColeta_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created historicoStatusColeta")
                        .delete("${new_historicoStatusColeta_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HistoricoStatusColeta entity").exec(scn);
