...
/**
 * Performance test for the FormaCobranca entity.
 */
public class FormaCobrancaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all formaCobrancas").get("/api/forma-cobrancas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formaCobranca")
                        .post("/api/forma-cobrancas")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formaCobranca_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created formaCobranca").get("${new_formaCobranca_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created formaCobranca").delete("${new_formaCobranca_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormaCobranca entity").exec(scn);
