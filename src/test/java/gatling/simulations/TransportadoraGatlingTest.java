...
/**
 * Performance test for the Transportadora entity.
 */
public class TransportadoraGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all transportadoras").get("/api/transportadoras").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new transportadora")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"inscricaoEstadual\": \"SAMPLE_TEXT\"" +
                                ", \"inscricaoMunicipal\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_transportadora_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created transportadora").get("${new_transportadora_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created transportadora").delete("${new_transportadora_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Transportadora entity").exec(scn);
