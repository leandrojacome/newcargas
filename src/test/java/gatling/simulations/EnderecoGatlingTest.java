 ...
 */
public class EnderecoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all enderecos").get("/api/enderecos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new endereco")
                        .post("/api/enderecos")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created endereco").get("${new_endereco_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created endereco").delete("${new_endereco_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Endereco entity").exec(scn);
