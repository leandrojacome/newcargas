...
/**
 * Performance test for the TipoVeiculo entity.
 */
public class TipoVeiculoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tipoVeiculos").get("/api/tipo-veiculos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tipoVeiculo")
                        .post("/api/tipo-veiculos")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tipoVeiculo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tipoVeiculo").get("${new_tipoVeiculo_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created tipoVeiculo").delete("${new_tipoVeiculo_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TipoVeiculo entity").exec(scn);
