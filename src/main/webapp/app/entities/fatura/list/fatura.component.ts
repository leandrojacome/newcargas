...
import { EntityArrayResponseType, FaturaService } from '../service/fatura.service';
...
export class FaturaComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['tipo', 'observacao', 'createdBy', 'lastModifiedBy'];
  ...
  trackId = (_index: number, item: IFatura): number => this.faturaService.getFaturaIdentifier(item);

  search(query: string): void {
    if (query && FaturaComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (FaturaComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.faturas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFatura[] | null): IFatura[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.faturaService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.faturaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
